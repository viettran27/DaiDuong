...
@router.get("/export")
async def get_qte():
    samples = [["1295", date.today(), "22I01", 8.109]]
    return export_columns("SL_CN_LA_DD", "sanluongcongnhanla", engine_2, samples) 
...
async def filter_cn_cat(db: Session = Depends(get_db_2), NGAY: date | None = None, CHUYEN: str | None = None, MST: str | None = None, page: int = 1, size: int = 20):
    ...
    return execute_filtered_query(db, "SL_CN_LA_DD", "NGAY", filters, page, size)
...
async def upload_excel(file: UploadFile = File(...)):    
    ...
    return validate_and_import_excel(file, engine_2, "SL_CN_LA_DD", dtype, columns_to_validate)